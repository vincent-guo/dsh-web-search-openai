...
- Auxiliary session event: append `web/openai-search-request` only when the running build's `KNOWN_SESSION_EVENT_TYPES` contains the type, and always pass `{ ignorable: true }`. Appending an unknown, non-ignorable type makes the harness refuse the whole log on the next cold load (`SessionFormatUnsupportedError`).
...
- Peer: `@deepseek-ai/dsh-session` (`^0.1.0-rc.6`) — read-only use of the build's `KNOWN_SESSION_EVENT_TYPES` vocabulary.
...
1. Change code/docs; update both READMEs for any config key, default, error, wire-field, or session-event behavior change in the same commit.
